...
export default function StoreProductModal({ product, onClose }: StoreProductModalProps) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/merchant-products/${product.id}`)
      .then((res) => res.json())
      .then((data) => {
    ...
    fetch(`/api/merchant-products/${product.id}/videos`)
      .then((res) => res.json())
      .then((data) => {
  ...
  return (
    <>
      ...
      {/* Modal */}
      <div className="fixed inset-x-0 bottom-0 z-50 max-h-[90vh] overflow-y-auto bg-card rounded-t-2xl animate-slide-up">
        {/* Handle bar */}
        <div className="sticky top-0 z-10 flex justify-center py-3 bg-card rounded-t-2xl">
        ...
        <div className="px-4 pb-8">
          {/* Product image */}
          {product.imageUrl && (
            <div className="relative aspect-square max-w-sm mx-auto rounded-xl overflow-hidden bg-surface mb-4">
              <Image
